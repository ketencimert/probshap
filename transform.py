    ...
    def __init__(self,
        ...
        self.loc_net = nn.Sequential(
            *create_feedforward_layers(
                int((d_in // d_patch) ** 2), d_emb*3, d_in * d_in, 
                n_layers, activation, p, norm
                )[0]
            )

    def forward(self, x_enc):
        return self.loc_net(x_enc)
...
def train(args, model, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, _) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        x_dec = model(data)
        #what you are trying to minimize
        ...
        if batch_idx % args.log_interval == 0:
            ...
            # plt.imshow(x_dec[10].reshape(d_in,d_in).detach().cpu())
            plt.show()
    return model
...
def test(args, model, best_model, test_loader, best_score):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            plt.imshow(nn.Sigmoid()(x_dec)[10].reshape(d_in,d_in).detach().cpu())
            plt.show()
            loss_list.append(loss.item())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss', default='cb', type=str,
    ...
    if os.path.exists(
            './transform_{args.dataset}_{d_segment}_segments_{args.loss}.pt'
            ):
